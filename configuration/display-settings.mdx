...
<ParamField path="neo-git-graph.showCurrentBranchByDefault" type="boolean" default="false">
  Show the current branch by default when Git Graph is opened.
  
  When enabled, the graph will automatically filter to show only the current branch when you first open Git Graph. When disabled (default), all branches are shown.
</ParamField>
